...
const statistics = ref<{ totalProperties: number; totalBookings: number } | null>(null);
  ...
  <div class="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6">
    ...
    <div class="bg-white shadow-md rounded-2xl p-8 w-full max-w-2xl">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <!-- Total Properties -->
        <div class="flex flex-col items-center justify-center bg-gray-100 rounded-xl p-6 shadow-sm hover:shadow-md transition">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-blue-500 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
          ...
          <p class="text-3xl font-bold text-blue-600 mt-2">{{ statistics?.totalProperties ?? 0 }}</p>
          <p class="text-sm text-gray-500 mt-1">All registered properties</p>
        </div>

        <!-- Total Bookings -->
        <div class="flex flex-col items-center justify-center bg-gray-100 rounded-xl p-6 shadow-sm hover:shadow-md transition">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-green-500 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
            <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10m-11 4h12m-12 4h12M5 5h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2z" />
          </svg>
          <h3 class="text-xl font-semibold text-gray-800">Total Bookings</h3>
          <p class="text-3xl font-bold text-green-600 mt-2">{{ statistics?.totalBookings ?? 0 }}</p>
      ...
      <div v-if="error" class="text-center mt-4 text-blue-500">Welcome to TravelAPAP Accommodation/Bill Service</div>
    </div>
  </div>
</template>
